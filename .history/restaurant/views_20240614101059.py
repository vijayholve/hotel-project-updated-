...
from .forms import many_images_form
...
def update_restaurant(request,pk):
    ...
    if request.method == "POST":
        ...
        restaurant_obj.save()
        form=many_images_form(request.POST,request.FILES,instance=restaurant_obj)
        if files:=request.POST.getlist("image"):
            if form.is_valid():
                for img in files:
                    image = restaurant_obj.images_set.create(
        ...
        form=many_images_form(instance=restaurant_obj)
    content={"restaurant":restaurant_obj,"form":form}
...
def restaurant_data(request,pk):
    ...
    q= request.GET.get("q")
    content={}
    if images:=restaurant.images_set.all():
