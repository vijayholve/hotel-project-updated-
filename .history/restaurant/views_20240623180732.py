...
def order_dish(request,pk):
    ...
    rating=Reviews.objects.filter(dish=dishe).aggregate(    rating_count=Reviews.objects.filter(room=room).count()
    rating_count=rating=Reviews.objects.filter(dish=dishe).count()
    if request.method == "POST":
