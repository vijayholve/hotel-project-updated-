...
def _extracted_from_register_4(request):
    ...
    email_for_otp_verification([email],fullname,otp)
    otp_verificaton(request,otp)
    ...
    return redirect("home")
def otp_verificaton(request,first_otp):
    if request.method == "POST":
        req=request.POST
        second_otp=list(req.get("otp1"),req.get("otp2"),req.get("otp3"),req.get("otp4"))
        if second_otp == str(first_otp):
            return True
        messagFalsees.error(request,"enter valid otp")
        return 
