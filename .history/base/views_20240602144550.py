...
def restaurant_data(request,pk):
    ...
    if q is not None :
        filter_dishes=dishes.filter(Q(dishName__icontains=q) |
                             Q(description__icontains=q) |
                             Q(price__icontains=q) 
                             )
    
    if under_value := request.GET.get("under_value"):
        filter_dishes=dishes.filter(price__lte= int(under_value )    
    content={"restaurant":restaurant,"dishes":filter_dishes}
