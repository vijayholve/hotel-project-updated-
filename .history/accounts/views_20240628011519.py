...
def user_rooms_data(request,pk):
    ...
    for room in rooms:
        ...
        startdate=Booking.objects.filter(room=room)
        stdate = startdate.start_date  # This is already a datetime.date object
        end_date = startdate.end_date  # Assuming you have an end_date field similarly
        start_datetime = datetime.combine(stdate, datetime.min.time())
        end_datetime = datetime.combine(end_date, datetime.min.time())
        if room.id not in start_datetime:
            start_date[room.id]=0
        if start_datetime is not None:
            start_date[room.id]+=start_datetime
