...
def user_rooms_data(request,pk):
    ...
    for room in rooms:
        ...
        if room.id not in start_date:
            start_date[room.id]=0
        if startdate is not None:
            startdate[room.id]+=startdate
        start_date = r.start_date  # This is already a datetime.date object
        end_date = r.end_date  # Assuming you have an end_date field similarly

        # If you need to work with datetime objects, you can convert them
