...
def user_rooms_data(request,pk):
    ...
    rating_count={}
    for r in rooms:
        avg_review = bookings.objects.filter(dish=d).aggregate(Avg("review"))['review__avg']
        count_review = Reviews.objects.filter(room=r).count()
        if r.id not in review_booking:
            review_booking[r.id] = 0.0
            rating_count[r.id]= 0
        if avg_review is not None:
            review_booking[r.id] += float(avg_review)
            rating_count[r.id] += count_review
    context = {
        'rooms': rooms,
        "profile":profile,
