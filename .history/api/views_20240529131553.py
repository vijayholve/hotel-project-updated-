...
from .serializable import serializer_restaurant,serializer_dish,serializer_restaurant_dish
@api_view(["GET"])
def all_api(request):
...
def dish_api(request,pk):
    ...
    return Response(serialzer.data)
def restaurant_dish_api(request,id):
    restaurant=restaurants.objects.get(id=id)
    serializer=serializer_restaurant_dish
