...
def home_room(request):
    ...
    room_rating={}
    for room in rooms:
        avg_rate=Reviews.objects.filter(room=room).aggregate(Avg('review'))
        room_rating[room.id]=avg_rate if avg_rate is not None else 0
    content = {'rooms': rooms, 'booked_rooms': booked_rooms,"room_rating":room_rating}
